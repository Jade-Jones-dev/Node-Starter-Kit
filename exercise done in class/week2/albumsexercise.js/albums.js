...
let nextId = 1 + albumsData.reduce((n, album) => (n < album.albumId) ? album.albumId : n, 0);
...
app.get("/albums", function (req, res) {
    if (req.query.artistName) {
        const albums = albumsData.filter(a => a.artistName === req.query.artistName);
        if (albums.length === 0) {
            res.sendStatus(404);
            return;
        }
...
const compulsoryFields = ['artistName', 'collectionName', 'releaseDate', 'primaryGenreName'];
...
app.post("/albums", function (req, res) {
    ...
    allFields.forEach(fld => { if (req.body[fld]) { newEntry[fld] = req.body[fld] } });
...
app.put("/albums/:id", function (req, res) {
    ...
    allFields.forEach(fld => { if (req.body[fld]) { updatedEntry[fld] = req.body[fld] } });
